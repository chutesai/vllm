...
class StructuredOutputManager:
    ...
    def should_fill_bitmask(self, request: "Request") -> bool:
        ...
        if self.reasoner is not None:
            ...
            # If we've already determined reasoning has ended, return True
            ...
            if structured_req.reasoning_ended is None:
                ...
                if structured_req.reasoning_ended:
                    return True
            # If the model has generated tokens beyond the prompt and reasoning
            # hasn't ended yet according to our cached state, re-check using
            # all tokens. This handles the case where <think> is in the prompt
            # (from chat template) and </think> is generated by the model.
            if len(request.all_token_ids) > len(
                request.prompt_token_ids or []
            ) and self.reasoner.is_reasoning_end(list(request.all_token_ids)):
