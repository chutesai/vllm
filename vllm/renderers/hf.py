...
class HfRenderer(BaseRenderer[HfTokenizer]):
    ...
    def render_messages(
        ...
        # Store the resolved chat template and raw prompt for checksum computation
        prompt["_chat_template"] = resolved_chat_template  # type: ignore[typeddict-unknown-key]
        _tprompt = prompt_raw if isinstance(prompt_raw, str) else None
        prompt["_templated_prompt"] = _tprompt  # type: ignore[typeddict-unknown-key]
